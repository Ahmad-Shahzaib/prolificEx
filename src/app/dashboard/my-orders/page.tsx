...
export default function MyOrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) =>
      ...
      order.coin.toLowerCase().includes(normalized) ||
      order.network.toLowerCase().includes(normalized) ||
      order.status.toLowerCase().includes(normalized) ||
      order.payment_method.toLowerCase().includes(normalized) ||
      order.buyer.full_name.toLowerCase().includes(normalized) ||
      order.seller.full_name.toLowerCase().includes(normalized)
    );
  }, [orders, searchTerm]);
    ...
    <PageShell title="My Orders" description="View your active P2P orders and track order status.">
      <Card className="bg-[#111218] border border-white/5 rounded-3xl overflow-hidden">
        <CardContent className="p-4 sm:p-6">
          ...
          {loading && (
            <div className="text-sm text-[#9ca3af] mb-4">Loading orders…</div>
          )}
